...
class VarConv2d(Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=(1, 1), padding=(0, 0), bias=True):
        ...
        self.fold = nn.Fold(output_size=out_channels, kernel_size=self.kernel_size,
                            stride=self.stride, padding=self.padding)
        self.is_variational = True

    def forward(self, x):
        u = self.unfold(x).transpose(1, 2)
        fv = round((2*self.padding[0]+x.size(2) -
                    self.kernel_size[0]+1) / self.stride[0])
        fh = round((2*self.padding[1]+x.size(3) -
        ...
        mu = torch.nn.functional.fold(self.mu_layer(
            u).transpose(1, 2), (fv, fh), (1, 1))
        if self.training and self.is_variational:
            logvar = torch.nn.functional.fold(
                self.logvar_layer(u).transpose(1, 2), (fv, fh), (1, 1))
